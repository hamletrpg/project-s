...
func basic_red_bullet_damage_handler(target: CharacterBody2D, source):
	...
	if "dots" in target.status_effects and target.dots_stacking_count <= 5:
		target.dots_amount_of_base_damage += 2
